  ...
  return (
    <div className="space-y-8">
      ...
      <div className="card-glass rounded-2xl p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-zinc-800/50">
          <div>
          ...
          <button
            onClick={handleReset}
            className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-medium text-zinc-400 bg-zinc-800/80 hover:bg-zinc-700/80 border border-zinc-700/50 transition self-start md:self-auto"
          >
              ...
              return (
                <button
                  ...
                  className={`p-3 rounded-xl text-left border transition-all duration-150 ${
                    isSelected 
                      ? 'border-emerald-600 bg-emerald-500/10/50 shadow-lg shadow-black/10 ring-1 ring-emerald-500' 
                      : 'border-zinc-700/50 bg-zinc-900/60 backdrop-blur-md hover:border-zinc-600/50 hover:bg-zinc-950/80/80'
                  }`}
                >
      ...
      <div className="card-glass rounded-2xl p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div>
            <h2 className="text-sm font-bold text-zinc-50 tracking-tight">Active Inference Models</h2>
            <p className="text-xs text-zinc-400">Check or uncheck architectures to evaluate concurrently.</p>
          </div>

          <div className="flex space-x-2 text-xs">
            <button 
              onClick={() => setSelectedModels({ xgboost: true, random_forest: true, neural_net: true })}
              className="text-emerald-600 hover:text-emerald-800 font-medium px-2 py-1 rounded hover:bg-emerald-500/10"
            >
              Select All
            </button>
            ...
            <button 
              ...
              className="text-zinc-400 hover:text-zinc-300 font-medium px-2 py-1 rounded hover:bg-zinc-800/80"
            >
              Clear
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <label 
            className={`flex items-start p-3.5 rounded-xl border cursor-pointer transition select-none ${
              selectedModels.xgboost 
                ? 'bg-emerald-500/10/40 border-emerald-500 shadow-lg shadow-black/10' 
                : 'bg-zinc-900/60 backdrop-blur-md border-zinc-700/50 hover:bg-zinc-950/80'
            }`}
          >
            <input 
              type="checkbox" 
              ...
              className="mt-0.5 w-4 h-4 text-emerald-600 rounded border-zinc-600/50 focus:ring-emerald-500" 
            />
            <div className="ml-3">
              <div className="flex items-center space-x-1.5">
                <span className="text-xs font-bold text-zinc-50">XGBoost</span>
                <span className="text-[10px] font-mono font-semibold bg-zinc-800/80 px-1.5 py-0.5 rounded text-zinc-400 border border-zinc-700/50">v2.4</span>
              </div>
              <p className="text-[11px] text-zinc-400 mt-0.5">Gradient Boosted Trees (Lowest Latency ~14ms)</p>
            </div>
          </label>
          ...
          >
            ...
            <div className="ml-3">
              <div className="flex items-center space-x-1.5">
                <span className="text-xs font-bold text-zinc-50">Random Forest</span>
                <span className="text-[10px] font-mono font-semibold bg-zinc-800/80 px-1.5 py-0.5 rounded text-zinc-400 border border-zinc-700/50">v1.8</span>
              </div>
              <p className="text-[11px] text-zinc-400 mt-0.5">200 Decision Trees (Balanced Variance ~26ms)</p>
            </div>
          </label>
          ...
          >
            ...
            <div className="ml-3">
              <div className="flex items-center space-x-1.5">
                <span className="text-xs font-bold text-zinc-50">Neural Net (MLP)</span>
                <span className="text-[10px] font-mono font-semibold bg-zinc-800/80 px-1.5 py-0.5 rounded text-zinc-400 border border-zinc-700/50">v3.1</span>
              </div>
              <p className="text-[11px] text-zinc-400 mt-0.5">4-Layer Deep Classifier (Non-linear ~38ms)</p>
            </div>
          </label>
        </div>
      </div>
      ...
      <div className="card-glass rounded-2xl p-6">
        <h2 className="text-sm font-bold text-zinc-50 uppercase tracking-wider text-xs mb-4 pb-2 border-b border-zinc-800/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="text-xs font-semibold text-zinc-300">Account Balance</label>
              <span className="text-xs font-mono font-bold text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-100">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="text-xs font-semibold text-zinc-300">Call Duration</label>
              <span className="text-xs font-mono font-bold text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-100">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="text-xs font-semibold text-zinc-300">Client Age</label>
              <span className="text-xs font-mono font-bold text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-100">
                {formData.age} yrs
              </span>
            </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="text-xs font-semibold text-zinc-300">Campaign Contacts</label>
              <span className="text-xs font-mono font-bold text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-100">
          ...
          {/* Dropdown 1: Job */}
          <div>
            <label className="text-xs font-semibold text-zinc-300 block mb-1.5">Job Category</label>
            <select
              value={formData.job}
              onChange={(e) => handleInputChange('job', e.target.value)}
              className="w-full bg-zinc-900/60 backdrop-blur-md border border-zinc-700/50 rounded-xl px-3 py-2 text-xs text-zinc-200 focus:outline-none focus:ring-1 focus:ring-emerald-500 shadow-lg shadow-black/10"
            >
          ...
          <div>
            <label className="text-xs font-semibold text-zinc-300 block mb-1.5">Education</label>
            <select
          ...
          <div>
            <label className="text-xs font-semibold text-zinc-300 block mb-1.5">Housing Loan</label>
            <select
          ...
          <div>
            <label className="text-xs font-semibold text-zinc-300 block mb-1.5">Prior Campaign Outcome</label>
            <select
        ...
        <div className="mt-6 pt-4 border-t border-zinc-800/50 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-zinc-50 rounded-xl text-xs font-bold shadow-xl shadow-black/20 shadow-emerald-200 transition disabled:opacity-50"
          >
        ...
        <div className={`p-4 rounded-2xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
          ...
          <div className="flex items-center space-x-3">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg ${
              consensusDecision ? 'bg-emerald-600 text-zinc-50 shadow-xl shadow-black/20 shadow-emerald-200' : 'bg-rose-600 text-zinc-50 shadow-xl shadow-black/20 shadow-rose-200'
            }`}>
            ...
            <div>
              ...
              <h3 className="text-base font-black tracking-tight text-zinc-50">
                {consensusDecision 
          ...
          <div className="flex items-center space-x-4 text-xs font-medium">
            <div>
              <span className="text-zinc-400 block text-[10px]">Avg Confidence</span>
            ...
            <div className="border-l border-zinc-700/50 pl-4">
              <span className="text-zinc-400 block text-[10px]">Mean Latency</span>
              <span className="font-bold font-mono">
                {activeCount > 0 
      ...
      {activeCount === 0 ? (
        <div className="card-glass rounded-2xl p-12 text-center text-zinc-400 text-xs">
        ...
        <div className={`grid grid-cols-1 gap-6 ${activeCount === 1 ? 'max-w-md mx-auto' : activeCount === 2 ? 'md:grid-cols-2' : 'lg:grid-cols-3'}`}>
          {activeKeys.map((key) => {
            ...
            return (
              ...
              >
                {/* Top indicator bar */}
                <div className={`absolute top-0 left-0 right-0 h-1.5 ${isApproved ? 'bg-emerald-500' : 'bg-zinc-400'}`} />
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <div className="flex items-center space-x-1.5">
                        <h3 className="font-bold text-base text-zinc-50">{m.name}</h3>
                        <span className="font-mono text-[10px] font-semibold text-zinc-400 bg-zinc-800/80 px-1.5 py-0.5 rounded border border-zinc-700/50">
                          {m.version}
                        </span>
                      </div>
                      <span className="text-[11px] text-zinc-400 font-medium">{m.tag}</span>
                    </div>

                    {/* Latency Pill */}
                    <div className="flex items-center space-x-1 px-2.5 py-1 rounded-lg bg-zinc-950/80 border border-zinc-700/50 text-zinc-400 text-xs font-mono font-medium">
                  ...
                  <div className={`p-4 rounded-xl border-2 mb-4 transition-all ${
                    ...
                    <div className="mt-3">
                      <div className="flex justify-between text-xs font-semibold mb-1">
                        ...
                        <span className="font-mono text-zinc-50 font-bold">
                          {m.confidence}%
                        </span>
                      </div>
                      <div className="h-2 w-full bg-zinc-700/80 rounded-full overflow-hidden">
                        <div 
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {m.features.map((f, i) => (
                        <div key={i} className="flex justify-between items-center text-xs p-2 rounded-lg bg-zinc-950/80 border border-zinc-800/50">
                          <span className="text-zinc-400 font-medium">{f.name}</span>
                          <span className={`font-mono font-bold text-[11px] ${
                            f.weight.startsWith('+') ? 'text-emerald-600' : 'text-zinc-400'
                          }`}>
                            {f.weight}
                          </span>
                        </div>
