...
export default function PredictionResult({ result, loading }) {
  if (loading) {
    return (
      <div className="glass-panel rounded-2xl p-8 border border-zinc-800 flex flex-col items-center justify-center min-h-[460px] text-center">
        <div className="relative mb-4">
          <div className="w-16 h-16 rounded-full border-4 border-emerald-500/20 border-t-emerald-500 animate-spin"></div>
          <Zap className="w-6 h-6 text-emerald-400 absolute top-1/2 left-1/2 -tranzinc-x-1/2 -tranzinc-y-1/2 animate-pulse" />
        </div>
        <h3 className="text-lg font-semibold text-zinc-50">Running Model Inference...</h3>
  ...
  if (!result) {
    return (
      <div className="glass-panel rounded-2xl p-8 border border-zinc-800 flex flex-col items-center justify-center min-h-[460px] text-center">
        <div className="w-16 h-16 rounded-2xl bg-zinc-800/60 border border-zinc-700/60 flex items-center justify-center text-zinc-400 mb-4">
          <TrendingUp className="w-8 h-8 text-zinc-400" />
        </div>
        <h3 className="text-lg font-semibold text-zinc-200">Awaiting Customer Input</h3>
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-2 text-xs text-zinc-400">
          <span className="px-2.5 py-1 rounded-md bg-zinc-800/80 border border-zinc-700">42 Transformed Features</span>
          <span className="px-2.5 py-1 rounded-md bg-zinc-800/80 border border-zinc-700">StandardScaler Normalized</span>
          <span className="px-2.5 py-1 rounded-md bg-zinc-800/80 border border-zinc-700">L2 Logistic Model</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`glass-panel rounded-2xl p-6 border transition-all duration-300 ${
        isSubscribed 
          ? 'border-emerald-500/40 bg-gradient-to-br from-emerald-950/40 via-zinc-900/90 to-zinc-950 glow-emerald' 
          : 'border-zinc-800 bg-gradient-to-br from-zinc-900/90 via-zinc-900/70 to-zinc-950'
      }`}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-zinc-800/80">
          <div className="flex items-center space-x-3.5">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-lg ${
              isSubscribed ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40' : 'bg-zinc-800 text-zinc-400 border border-zinc-700'
            }`}>
              {isSubscribed ? <CheckCircle2 className="w-7 h-7" /> : <XCircle className="w-7 h-7" />}
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-xs uppercase tracking-wider font-semibold text-zinc-400">Prediction Outcome</span>
              ...
              <h2 className="text-2xl font-bold text-zinc-50 tracking-tight mt-0.5">
          ...
          <div className="sm:text-right">
            <span className="text-xs text-zinc-400 block font-medium">Model Confidence</span>
            <div className="flex items-baseline sm:justify-end space-x-1">
              <span className={`text-4xl font-extrabold tracking-tight ${isSubscribed ? 'text-emerald-400' : 'text-zinc-200'}`}>
                {probYes}%
              </span>
              <span className="text-xs text-zinc-400 font-medium">positive</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 space-y-2">
          ...
          <div className="h-2.5 w-full bg-zinc-800/80 rounded-full overflow-hidden flex p-0.5 border border-zinc-700/50">
            <div 
              className="bg-gradient-to-r from-emerald-500 to-teal-400 h-full rounded-full transition-all duration-700 ease-out"
              style={{ width: `${probYes}%` }}
            ></div>
            <div 
              className="bg-zinc-700/60 h-full rounded-full transition-all duration-700 ease-out"
              style={{ width: `${probNo}%` }}
            ></div>
          </div>
        </div>
        ...
        {result.customer_summary && (
          <div className="mt-5 pt-4 border-t border-zinc-800/70 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div className="bg-zinc-900/60 p-2 rounded-lg border border-zinc-800">
              <span className="text-zinc-400 block">Job / Role</span>
              <span className="font-semibold text-zinc-200">{result.customer_summary.job}</span>
            </div>
            <div className="bg-zinc-900/60 p-2 rounded-lg border border-zinc-800">
              <span className="text-zinc-400 block">Current Balance</span>
              <span className="font-semibold text-zinc-200">{result.customer_summary.balance}</span>
            </div>
            <div className="bg-zinc-900/60 p-2 rounded-lg border border-zinc-800">
              <span className="text-zinc-400 block">Call Duration</span>
              <span className="font-semibold text-zinc-200">{result.customer_summary.contact_duration}</span>
            </div>
            <div className="bg-zinc-900/60 p-2 rounded-lg border border-zinc-800">
              <span className="text-zinc-400 block">Campaign Touches</span>
              <span className="font-semibold text-zinc-200">{result.customer_summary.campaign_calls} contact(s)</span>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="space-y-2.5">
            {result.top_positive_factors && result.top_positive_factors.length > 0 ? (
              result.top_positive_factors.map((factor, i) => (
                <div key={i} className="flex items-center justify-between text-xs bg-zinc-900/70 p-2.5 rounded-xl border border-emerald-500/20">
                  <span className="text-zinc-200 font-medium truncate mr-2">{factor.label}</span>
              ...
              <p className="text-xs text-zinc-400 py-3 text-center">No major positive drivers detected</p>
            )}
          </div>
        </div>
          ...
          <div className="space-y-2.5">
            {result.top_negative_factors && result.top_negative_factors.length > 0 ? (
              result.top_negative_factors.map((factor, i) => (
                <div key={i} className="flex items-center justify-between text-xs bg-zinc-900/70 p-2.5 rounded-xl border border-rose-500/20">
                  <span className="text-zinc-200 font-medium truncate mr-2">{factor.label}</span>
              ...
              <p className="text-xs text-zinc-400 py-3 text-center">No major negative resistance factors</p>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            {result.recommendations.map((rec, i) => {
              const borderColors = {
                ...
                action: 'border-emerald-500/30 bg-emerald-950/20 text-emerald-300',
              ...
              return (
                ...
                >
                  <div className="flex items-start space-x-2.5">
                    <span className="font-semibold text-xs text-zinc-50 block mt-0.5">
                      {rec.title}
                    </span>
                  </div>
                  <p className="text-xs text-zinc-300 mt-1 pl-0.5 leading-relaxed">
                    {rec.message}
                  </p>
                </div>
              );
