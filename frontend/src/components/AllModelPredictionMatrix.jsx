...
export default function AllModelPredictionMatrix() {
  ...
  return (
    <div className="card-clean rounded-2xl p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-5 border-b border-slate-100">
        <div>
          <div className="flex items-center space-x-2 text-indigo-600 mb-1">
            ...
            <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 ml-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
        ...
        <div>
          <div className="flex justify-between text-xs font-semibold mb-1">
            <span className="text-slate-700">Bank Account Balance:</span>
            <span className="font-mono text-indigo-700 bg-white px-2.5 py-0.5 rounded-md border border-slate-200 font-bold text-sm">
          ...
          <input 
            ...
            className="w-full h-2.5 bg-slate-200 rounded-lg cursor-pointer accent-indigo-600"
          />
        ...
        <div>
          <div className="flex justify-between text-xs font-semibold mb-1">
            <span className="text-slate-700">Phone Call Duration:</span>
            <span className="font-mono text-indigo-700 bg-white px-2.5 py-0.5 rounded-md border border-slate-200 font-bold text-sm">
      ...
      <div className={`p-5 rounded-2xl border-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all duration-300 ${
        ...
        <div className="flex items-center space-x-4">
          <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center font-black text-xl shadow-md ${
            isConsensusApproved 
        ...
        <div className="flex items-center space-x-4 text-xs font-medium self-end sm:self-auto bg-white/70 px-4 py-2 rounded-xl border border-slate-200/50">
          <div>
            <span className="text-slate-500 block text-[10px] uppercase font-bold">Avg Confidence</span>
            <span className="font-bold font-mono text-slate-900 text-sm">{avgConfidence}%</span>
          </div>
          <div className="border-l border-slate-200 pl-4">
            <span className="text-slate-500 block text-[10px] uppercase font-bold">Mean Latency</span>
            <span className="font-bold font-mono text-slate-900 text-sm">{meanLatency} ms</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {modelsList.map((m, idx) => {
          ...
          return (
            <div 
              key={idx}
              className={`card-clean rounded-2xl p-5 flex flex-col justify-between border-t-4 ${m.borderColor} card-clean-hover relative`}
            >
              <div>
                {/* Header */}
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between text-[11px] font-semibold">
                    <span className="text-slate-600">Chance of YES:</span>
                    <span className="font-mono font-bold text-slate-900">{m.prob}%</span>
                  </div>
              ...
              <div className="mt-4 pt-2.5 border-t border-slate-100 text-[11px] text-slate-500 truncate" title={m.topFactor}>
                <span className="font-semibold text-slate-700">Key Factor:</span> {m.topFactor}
              </div>
            </div>
          );
        })}
